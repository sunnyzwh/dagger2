    ...
    if (e.type == 'mouseenter' || e.type == 'mouseleave') {
        ...
        if(opt_type == 'order'){
            if (e.type == 'mouseenter') {
                obj.find(".sortoptions").show();
            } else {
                obj.find(".sortoptions").hide();
            }
        }
    }
}
        ...
        show : function(content, context) {
            ...
            this.tipContainer.css({
             ...
             this.tipContainer.animate({'top': (showTop - 25) + 'px','opacity': 1}, 250, 'swing', function(){
                    setTimeout(function(){tipObj.hidden()},3000);
             });
        },
        hidden : function() {
    ...
    var loading = plugins.loading,
        ...
        $.extend(optionsBtnEventSys.prototype, {
            init : function(){
                var me = this;
                this._resetElms();
                $('body').on('click', buttonOptionsSelecter, function(){me.clickCallback(this)});
                $('body').on('mouseenter', buttonOptionsSelecter, bindOpt);
                $('body').on('mouseleave', buttonOptionsSelecter, bindOpt);
                /*
                $('body').on({
                    selector: buttonOptionsSelecter,
            ...
            refreshORsearchCallback : function(search,data, url) {
                var rhasParam = /\?/g,
                    ...
                    action = action + "&_" + (new Date()).getTime();
            ...
            addORupdateCallback : function(update){
                ...
                setTimeout(function(){
                    ...
                    //this.loading.hidden();
                    loading.hidden();
                    //update input value show，like file & img
                    if (update) {
                        $(".dagger_input_value_show").show();
                    } else {
                        $(".dagger_input_value_show").hide();
                    }
                    $("#ui-fail-msg").remove();
            ...
            _initEditor : function(name){
               ...
               onGetSuccess : function(data){
                ...
                for(name in results){
                    try{
                        ...
                        if(/^SELECTED:\[\'.*\'\]$/.test(temp)){
                        ...
                        }else if(/^FILE:\[\'.*\'\]$/.test(temp)){
                            temp = temp.replace(/^FILE:\[\'(.*)\'\]/g, "$1");
                            $("#show_file_" + name).attr('href', temp);
                            $("#show_file_" + name).show();
                            ...
                            temp = new Array(temp);
                            if (!$("[name='" + name + "']", this.form).is(":file")) {
                                $("[name='" + name + "']", this.form).val(temp);
                            } 
                        }
               ...
               submitCallback : function(json, status, xhr, form){
                    ...
                    */
                    !$('#ui-fail-msg').get(0) && (this.failMsg = $('<div class="ui-fail-msg" id="ui-fail-msg">' + json.status.msg + '</div>'));
                    this.form.prepend(this.failMsg);
                    var input = this.form.find("input[name='" + json.data.field + "'],textarea[name='" + json.data.field + "'],select[name='" + json.data.field + "'],radio[name='" + json.data.field + "'],checkbox[name='" + json.data.field + "']");
                    ...
                    //this.showMsg(msg, form.find("input[name='" + json.data.field + "']"));
                    input.focus();
